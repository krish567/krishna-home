...
export default function PaperViz_08({ title, authors, type }: { title: string; authors: string; type: string }) {
        ...
        {stages.map((s, i) => (
          <div key={i} className="flex flex-col items-center gap-1">
            <div className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold" style={{
              background: i < Math.floor(progress / 25) ? "#f59e0b" : "#334155",
              color: i < Math.floor(progress / 25) ? "#fff" : "#64748b",
              boxShadow: i < Math.floor(progress / 25) ? "0 0 20px #f59e0b60" : "none"
            }}>{i + 1}</div>
            <span className="text-xs" style={{color: i < Math.floor(progress / 25) ? "#f59e0b" : "#64748b"}}>{s}</span>
          </div>
        ))}
      </div>
      <div className="w-full h-2 rounded-full bg-slate-700 overflow-hidden">
        <div className="h-full rounded-full transition-all" style={{width: progress + "%", background: "linear-gradient(90deg, #f59e0b, #ef4444)", boxShadow: "0 0 10px #f59e0b60"}} />
      </div>
